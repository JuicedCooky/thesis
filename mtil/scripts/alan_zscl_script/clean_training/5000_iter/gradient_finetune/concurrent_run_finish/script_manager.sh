...
FOLDER_PATH="ckpt/clean/5000_iter/gradient_finetune_continue"
ITERATIONS=5000
JOB_NAME="gradient_finetune"
...
do
    ...
    TRAIN_CMD=(
        sbatch
        --parsable
        --job-name=${JOB_NAME}_train_${dataset}_backup
        --dependency=afterok:$PREV_JOB_ID
        $SCRIPT_DIR/train.sh
        $dataset
        $FOLDER_PATH
    ...
    PREV_JOB_ID=$( "${TRAIN_CMD[@]}" | tr -d ' \n' )
    echo "Queued backup training - dataset: $dataset - Job ID: $PREV_JOB_ID"
